...
function Workspace() {
  ...
  const [
    ...
    setEditPopUpLeftSideWorkspacesStatus,
  ] = useState(false);
      ...
      <div className="flex w-screen flex-1 h-screen">
            ...
            workspaces.map((workspace) => (
              ...
              >
                ...
                >
                  {active === workspace.id && (
                    <img
                      onClick={() => setEditPopUpLeftSideWorkspacesStatus(true)}
                      className="fw-4 h-4 ml-4 cursor-pointer hover:rotate-90 duration-300"
                      src={edit}
                      alt="edit button"
                    />
                ...
                <button onClick={() => trelloAPI.deleteWorkspace(workspace.id)}>
                  <img
                    className="fw-4 h-4 w-4 flex ml-4 cursor-pointer hover:rotate-90 duration-300"
                    src={delete_button}
                    alt="delete button"
                  />
                </button>
        ...
        <div className="flex flex-1 bg-gradient-to-r from-red-700/75 to-indigo-700/75 h-full scroll-smooth">
          <div>
            {boards.length ? (
              boards.map((board) => (
                <div
                  key={board.id}
                  className="flex  w-96 py-2 rounded-lg bg-gray-300 bg-opacity-60 m-3 justify-center items-center"
                >
                  <button
                    className="flex items-center justify-start"
                    onClick={() => handleBoardClick(workspaces, boards)}
                  >
          ...
          {active ? (
            <div>
              <button
                onClick={() => createBoard(active)}
                className="flex w-96 py-2 rounded-lg bg-gray-300 bg-opacity-60 cursor-pointer m-3"
              >
                <div className="flex-1 text-center italic opacity-60">
                  Create board
                </div>
              </button>
            </div>
