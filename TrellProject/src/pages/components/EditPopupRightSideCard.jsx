import React from 'react';
import { useNavigate } from 'react-router-dom';
import { useState , useEffect} from 'react';
import * as api from "../../api/api";

function EditPopupRightSideCard({setPopUpStatus, cardId}) {
    const [newName, setNewName] = useState(""); // State to hold the new name input by the user
    const [newDescription, setNewDescription] = useState(""); // State to hold the new description input by the user
    const trelloAPI = new api.TrelloAPI();
    const navigate = useNavigate();

    // Function to handle name update
    const handleUpdateCardName = async (e) => {
        e.preventDefault(); // Prevent the default form submission behavior
    
        const paramsToUpdate = { name: newName, desc: newDescription }; // Directly use newName and newDescription
    
        try {
            const response = await trelloAPI.updateCard(cardId, paramsToUpdate);
            // Consider closing the popup or showing a success message here
        } catch (error) {
            console.error("Failed to update the card:", error);
            // Handle the error, e.g., show an error message
        }
        //reload page
        window.location.reload();
    };
    

    return (

        <form onSubmit={handleUpdateCardName}>
            <div className="absolute bg-black opacity-85 h-screen w-screen inset-0 flex items-center justify-center">
                <div className="flex flex-col w-96 py-2 rounded-lg bg-white">
                    <div className="flex items-center justify-start">
                        <div className="flex-1 text-center">Name of the card :</div>
                        <div className="flex rounded-md mr-2 w-5 ring-1 ring-offset-1 ring-black">
                            <button onClick={() => setPopUpStatus(false)}
                                    className="flex items-center justify-center w-full h-full">
                                X
                            </button>
                        </div>
                    </div>
                    <div className="flex justify-center mb-2">
                        <input
                            className="flex-1 ml-2 mr-2 mt-2 rounded-md ring-1 ring-inset ring-black bg-transparent py-1.5 pl-1 text-gray-900 placeholder:text-gray-400 sm:text-sm sm:leading-6"
                            type="text"
                            placeholder="New Board name ..."
                            onChange={(e) => setNewName(e.target.value)} // Update state on input change
                        />
                    </div>
                    <div className="flex items-center justify-start">
                        <div className="flex-1 text-center">Description of the card :</div>
                    </div>
                    <div className="flex justify-center mb-2">
                        <input
                            className="flex-1 ml-2 mr-2 mt-2 rounded-md ring-1 ring-inset ring-black bg-transparent py-1.5 pl-1 text-gray-900 placeholder:text-gray-400 sm:text-sm sm:leading-6"
                            type="text"
                            placeholder="New Decription ..."
                            onChange={(e) => setNewDescription(e.target.value)}
                        />
                    </div>
                    <div className="flex justify-between pt-2">
                        <button className="bg-blue-500 text-white px-4 py-2 rounded ml-10">validate</button>
                    </div>
                </div>
            </div>
        </form>
    );
}

export default EditPopupRightSideCard;