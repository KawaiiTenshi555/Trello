...
function EditPopupListName({ setPopUpStatus, listId }) {
    ...
    const handleUpdatelistName = async (e) => {
        e.preventDefault(); // Prevent the default form submission behavior
        try {
            const response = await trelloAPI.updateList(listId, newName);
        } catch (error) {
            console.error("Failed to update the list name:", error);
    ...
    return (
        ...
        <form onSubmit={handleUpdatelistName}>
            <div className="absolute bg-black opacity-85 h-screen w-screen inset-0 flex items-center justify-center">
                <div className="flex flex-col w-96 py-2 rounded-lg bg-white">
                    <div className="flex items-center justify-start">
                        <div className="flex-1 text-center">Name of the list :</div>
...
export default EditPopupListName;