...
function Table() {
    ...
    const [EditPopUpLeftSideBoardsStatus, setEditPopUpLeftSideBoardsStatus] = useState(false);
    const [EditPopUpRightSideCardStatus, setEditPopUpRightSideCardStatus] = useState(false);
        ...
        <div className="overflow-x-hidden overflow-y-hidden h-screen">
            ...
            <div className="flex w-screen flex-1 h-screen">
                ...
                <div className="flex flex-col w-1/5 bg-gray-200 scroll-smooth border-r-2 border-black">
                    ...
                    {boards.length ? (
                        boards.map((board) => (
                            <div key={board.id} className="flex justify-center content-center ml-3 mr-3 mb-3">
                                <button onClick={() => handleWorkspaceClick(board)}
                                    ...
                                    <img onClick={() => setEditPopUpLeftSideBoardsStatus(true)}
                                ...
                                <div className="flex flex-col w-96 py-2 rounded-lg bg-gray-300 bg-opacity-60 m-3">
                                    <>
                                        <div className="flex items-center justify-start">
                                            ...
                                            <button className="flex-1 mr-8 ml-8 text-center" onClick={() => updateList(singleCards)}>
                                                {singleCards.name}
                                            </button>
                                            <img
                                        ...
                                        {singleCard.filter((card) => card.idList === singleCards.id).map((card) => (

                                            <div key={card.id} className="flex flex-col w-90 py-2 rounded-lg bg-gray-300 bg-opacity-60 m-3">
                                                <div className="flex items-center justify-start">
                                                    <img
                    ...
                    <div>
                        {createsList ? (
                            <div className="flex w-96 py-2 rounded-lg bg-gray-300 bg-opacity-60 cursor-pointer m-3">
                                <button onClick={() => createList()} className="flex-1 text-center italic opacity-60">Add Lists</button>
                            </div>
                        ) : null}
                    </div>
                </div>
